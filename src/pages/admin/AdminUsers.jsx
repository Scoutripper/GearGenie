        ...
        <div className="space-y-6">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div>
                    ...
                    <p className="text-slate-500 text-sm mt-1">Manage registered users from Supabase.</p>
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {loading ? (
                    ...
                    <div className="col-span-full py-12 text-center text-slate-500">No users found.</div>
            ...
            {totalPages > 1 && (
                <div className="flex items-center justify-center gap-3">
                    <button onClick={() => setCurrentPage(p => Math.max(1, p - 1))} disabled={currentPage === 1} className="px-4 py-2 border rounded-lg text-sm disabled:opacity-50">Previous</button>
                    <span className="text-sm text-slate-500">Page {currentPage} of {totalPages}</span>
                    <button onClick={() => setCurrentPage(p => Math.min(totalPages, p + 1))} disabled={currentPage === totalPages} className="px-4 py-2 border rounded-lg text-sm disabled:opacity-50">Next</button>
                </div>
            )}
                    ...
                    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
                        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="bg-white rounded-2xl max-w-md w-full p-6">
                            <div className="flex items-center justify-between mb-6">
                                ...
                                <button onClick={() => setSelectedUser(null)} className="p-2 hover:bg-slate-100 rounded-lg"><X /></button>
                            </div>
                            <div className="text-center mb-6">
                                <img src={selectedUser.avatar_url || `https://ui-avatars.com/api/?name=${selectedUser.email}`} className="w-20 h-20 rounded-full mx-auto mb-3 object-cover" />
                            ...
                            <div className="space-y-4">
                                <div className="p-3 bg-slate-50 rounded-xl text-sm">
                                    ...
                                    <p className="font-medium text-slate-800">{selectedUser.about_yourself || 'No info provided.'}</p>
                                </div>
                                <div className="p-3 bg-slate-50 rounded-xl text-sm">
                                    <p className="text-slate-500">Role</p>
                                    <p className="font-medium text-slate-800 capitalize">{selectedUser.role}</p>
                                </div>
                            </div>
                            <button onClick={() => setSelectedUser(null)} className="w-full mt-6 py-3 bg-[#4ec5c1] text-white rounded-xl font-bold">Close</button>
                        </motion.div>
                    </div>
                )}
            </AnimatePresence>
